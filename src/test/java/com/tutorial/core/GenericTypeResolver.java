...
public abstract class GenericTypeResolver {
	 ...
	 */
	public static Class<?> resolveParameterType(MethodParameter parameter, Class<?> clazz) {
		Type genericType = getTargetType(parameter);
		Assert.notNull(clazz, "Class must not be null");
		Map<TypeVariable<?>, Type> typeVariableMap = getTypeVariableMap(clazz);
		Type rawType = getRawType(genericType, typeVariableMap);
		Class<?> result = (rawType instanceof Class ? (Class<?>) rawType : parameter.getParameterType());
		parameter.setParameterType(result);
		parameter.typeVariableMap = typeVariableMap;
		return result;
	}
		...
		if(type.getRawType() instanceof Class) {
			...
			for(int i = 0; i < actualTypeArguments.length; i++) {
				...
				if(actualTypeArgument instanceof Class) {
					...
					//we have a type that is parameterzied at instantiation time 
