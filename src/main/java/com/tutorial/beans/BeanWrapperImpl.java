...
public class BeanWrapperImpl extends AbstractPropertyAccessor implements BeanWrapper {
	...
	private BeanWrapperImpl getNestedBeanWrapper(String nestedProperty) {
		...
		if(nestedBw == null || nestedBw.getWrappedInstance() != propertyValue) {
			...
			nestedBw = newNestedBeanWrapper(propertyValue, this.nestedPath + canonicalName + NESTED_PROPERTY_SEPERATOR);
	 ...
	 */
	protected BeanWrapperImpl newNestedBeanWrapper(Object obj, String nestedPath) {
		return new BeanWrapperImpl(obj, nestedPath, this);
	}
			...
			if(tokens.keys != null) {
				...
				for(int i = 0; i < tokens.keys.length; i++) {
					...
					if(value == null) {
						...
						// IMPORT :  Do not pass full property name in here - property editors
	...
	private Object growArrayIfNecessary(Object array, int index, String name) {
		...
		if(index >= length && index < this.autoGrowCollectionLimit) {
			...
			// this is not efficient because conversion may create a copy... set directly because we know it is assignable.
	...
	private void setPropertyValue(PropertyTokenHolder tokens, PropertyValue pv) 
		...
		if(tokens.keys != null) {
			...
			if(propValue.getClass().isArray()) {
				...
				// IMPORTANT : Don't pass full property name in here - property editors
				// must not kick in for map keys but rather only for map values;
	...
	public Object convertForProperty(Object value, String propertyName) throws TypeMisMatchException {
		...
		if(pd == null) {
			throw new InvalidPropertyException(getRootClass(), this.nestedPath + propertyName,
					"No property '" + propertyName + "'");
		} 
		return convertForProperty(propertyName, null, value, pd);
	}

	private Object convertForProperty(String propertyName, Object oldValue, Object newValue,
