...
class TypeConverterDelegate {
    ...
    public <T> T convertIfNecessary(Object newValue, Class<T> requiredType, 
    		MethodParameter methodParameter) throws IllegalArgumentException {
    	return convertIfNecessary(null, null, newValue, requiredType, 
    			(methodParameter != null ? new TypeDescriptor(methodParameter) : TypeDescriptor.valueOf(requiredType)));
    }
	...
	public <T> T convertIfNecessary(String propertyName, Object oldValue, Object newValue, Class<T> requiredType,
		...
		if(requiredType != null) {
			// Try to apply some standard type conversion rule if appropriate.
			if(convertedValue != null) {
				if(requiredType.isArray()) {
					...
					convertedValue = convertToTypeMap((Map) convertedValue, propertyName, requiredType, typeDescriptor);
				} 
				if(convertedValue.getClass().isArray() && Array.getLength(convertedValue) == 1) {
					convertedValue = Array.get(convertedValue, 0);
				}
				if(String.class.equals(requiredType) && ClassUtils.isPrimitiveOrWrapper(convertedValue.getClass())) {
					// We can stringify any primitive values...
	...
	private Object attemptToConvertStringToEnum(Class<?> requiredType, String trimmedValue, Object currentConvertedValue) {
		...
		if(Enum.class.equals(requiredType)) {
			...
			if(index > -1) {
				...
				try {
					...
					if(logger.isTraceEnabled()) {
						logger.trace("Field [" + fieldName + "] isn't an enum value for type [" + enumType + "]", e);
					}
				}
			}
		}
		...
		if(convertedValue == currentConvertedValue) {
			// Try field lookup as fallback: for JDK 1.5 enum or custom enum with 
			// values defined as static fields. Resulting value still needs to be checked, 
			// hence we don't return it right away.
			try {
	 ...
	 * @throws IllegalArgumentException if type conversion failed
	 */
	protected Object doConvertValue(Object oldValue, Object newValue, Class<?> requiredType,
	...
	protected Object doConvertTextValue(Object oldValue, String newTextValue, PropertyEditor editor) {
		try {
			...
			if(logger.isDebugEnabled()) {
				logger.debug("PropertyEditor [" + editor.getClass().getName() + "] does not support setValue call", e);
			}
		}
	...
	protected PropertyEditor findDefaultEditor(Class<?> requiredType, TypeDescriptor typeDescriptor) {
		...
		if(requiredType != null) {
			// No Custom editor -> check BeanWrapperImpl's default eidtors.
	...
	protected Object convertToTypeArray(Object input, String propertyName, Class<?> componentType) {
	...
	@SuppressWarnings({ "unchecked", "rawtypes" })
	protected Collection<?> convertToTypedCollection(Collection<?> original, String propertyName, Class<?> requiredType,
	...
	protected Map<?, ?> convertToTypeMap(Map<?, ?> original, String propertyName, Class<?> requiredType,
			TypeDescriptor typeDescriptor) {
