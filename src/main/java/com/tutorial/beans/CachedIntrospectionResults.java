...
public class CachedIntrospectionResults {
	...
	private static boolean isClassLoaderAccepted(ClassLoader classLoader) {
		// Iterator over array copy in order to avoid synchronization for the entire 
		...
		for(ClassLoader loader : acceptedLoaderArray) {
			if(isUnderneathClassLoader(classLoader, loader)) {
				return true;
			}
		}
		return false;
	...
	private CachedIntrospectionResults(Class<?> beanClass, boolean cacheFullMetadata) throws BeansException {
		try {
			...
			// Introspector does not use WeakReference as value of its WeakHashMap!
			...
			for(PropertyDescriptor pd : pds) {
				if(Class.class.equals(beanClass) && "classLoader".equals(pd.getName())) {
					// Ignore Class.getClassLoader() method - nobody need to binds to that.
					continue;
				}
				if(logger.isTraceEnabled()) {
