...
import com.tutorial.beans.factory.parsing.BeanComponentDefinition;
...
import com.tutorial.core.io.support.ResourcePatternUtils;
...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		...
		// any nested <beans> elements will cause recursion in this method. In order to 
		// propagate and preserve <beans> default-* attributes correctly, keep track of the 
		// current (parent) delegate, which may be null. Create the new (child) delegate with a 
		// reference to the parent for fallback purposes, then ultimately reset this.delegate back to its
		// original (parent) reference. this behavior emulates a stack of delegate without actually necessitating one.
	...
	protected void importBeanDefinitionResource(Element ele) {
		...
		if(absoluteLocation) {
			try {
				...
				getReaderContext().error("Failed to import definitions from URL location [" + location + "]", ele, e);
			}
		} else {
			...
			try {
				...
				if(relativeResource.exists()) {
					...
					String beseLocation = getReaderContext().getResource().getURL().toString();
					importCount = getReaderContext().getReader().loadBeanDefinitions(
							StringUtils.applyRelativePath(beseLocation, location), actualResources);
				}
				if(logger.isDebugEnabled()) {
				...
				getReaderContext().error("Failed to resolve current resource location", ele, e);
			}
		}
	...
	protected void processAliasRegistration(Element ele) {
		...
		String alias = ele.getAttribute(ALIAS_ELEMENT);
		boolean valid = true;
		if(!StringUtils.hasText(name)) {
			getReaderContext().error("Name must not be null", ele);
			valid = false;
		}
