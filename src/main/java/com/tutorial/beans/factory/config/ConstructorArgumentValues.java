...
public class ConstructorArgumentValues {
	 ...
	 * with the current value if demanded: see {@link com.tutorial.beans.Mergeable}.
	 * @param newValue the argument value in the form of a ValueHolder
	 */
	private void addOrMergeGenericArgumentValue(ValueHolder newHolder) {
		if(newHolder.getName() != null) {
			for(Iterator<ValueHolder> it = this.genericArgumentValues.iterator(); it.hasNext(); ) {
				ValueHolder currentValue = it.next();
				if(newHolder.getName().equals(currentValue.getName())) {
					if(newHolder.getValue() instanceof Mergeable) {
						Mergeable mergeable = (Mergeable) newHolder.getValue();
						if(mergeable.isMergeEnabled()) {
							newHolder.setValue(mergeable.merge(currentValue.getValue()));
						}
					}
					it.remove();
				}
		...
		this.genericArgumentValues.add(newHolder);
	}

	/**
	 * Add an argument value for the given index in the constructor argument list,
	 * merging the new value (typically a collection) with the current value
	 * if demanded: see {@link com.tutorial.beans.Mergeable}.
	...
	public boolean equals(Object obj) {
		...
		if(!(obj instanceof ConstructorArgumentValues)) return false;
		ConstructorArgumentValues other = (ConstructorArgumentValues) obj;
		if(this.genericArgumentValues.size() != other.genericArgumentValues.size() || 
				this.indexedArgumentValues.size() != other.indexedArgumentValues.size()) {
			return false;
		}
		...
		while(it1.hasNext() && it2.hasNext()) {
			...
			if(!vh1.contentEquals(vh2)) return false;
		}
