...
import com.tutorial.beans.factory.config.ConstructorArgumentValues.ValueHolder;
...
class ConstructorResolver {
		...
		if(explicitArgs != null) {
			...
			synchronized(mbd.constructorArgumentLock) {
				...
				if(constructorToUse != null && mbd.constructorArgumentsResolved) {
					...
					argsToUse = mbd.resolvedConstructorArgument;
					if(argsToUse == null) {
		...
		if(constructorToUse == null) {
			...
			for(int i = 0; i < candidates.length; i++) {
				...
				if(resolvedValues != null) {
					...
					// Explicit argument given -> arguments length must match exactly.
				...
				// choose this constructor if it represents the closet match.
	...
	public BeanWrapper instantiateUsingFactoryMethod(final String beanName, final RootBeanDefinition mbd, 
		...
		if(factoryMethodToUse == null || argsToUse == null) {
			...
			// Try all methods with this name to see if they match given arguments.
			factoryClass = ClassUtils.getUserClass(factoryClass);
			...
			for(int i = 0; i < candidates.length; i++) {
				...
				if(paramTypes.length >= minNrOfArgs) {
					...
					} else if(factoryMethodToUse != null && typeDiffWeight == minNrOfArgs) {
	...
	protected Object resolveAutowiredArgument(MethodParameter param, String beanName, Set<String> autowiredBeanNames,
		...
		return this.beanFactory.resolveDependency(
				new DependencyDescriptor(param, true), beanName, autowiredBeanNames, converter);
	}
	...
	private ArgumentsHolder createArgumentArray(String beanName, RootBeanDefinition mbd,
		...
		for(int paramIndex = 0; paramIndex < paramTypes.length; paramIndex++) {
			...
			if(valueHolder != null) {
				// we find a potential match - let's give it a try.
		...
		public void storeCache(RootBeanDefinition mbd, Object constructorOrFactoryMethod) {
			synchronized(mbd.constructorArgumentLock) {
				...
				if(this.resolveNecessary) {
					...
					mbd.resolvedConstructorArgument = this.arguments;
				}
			}
		}
	}
