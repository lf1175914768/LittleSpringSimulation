...
import com.tutorial.beans.propertyeditors.ByteArrayPropertyEditor;
import com.tutorial.beans.propertyeditors.CharArrayPropertyEditor;
import com.tutorial.beans.propertyeditors.CharacterEditor;
import com.tutorial.beans.propertyeditors.CharsetEditor;
import com.tutorial.beans.propertyeditors.ClassArrayEditor;
import com.tutorial.beans.propertyeditors.ClassEditor;
import com.tutorial.beans.propertyeditors.CurrencyEditor;
import com.tutorial.beans.propertyeditors.CustomBooleanEditor;
import com.tutorial.beans.propertyeditors.CustomCollectionEditor;
import com.tutorial.beans.propertyeditors.CustomMapEditor;
...
import com.tutorial.beans.propertyeditors.LocaleEditor;
import com.tutorial.beans.propertyeditors.PatternEditor;
import com.tutorial.beans.propertyeditors.PropertiesEditor;
import com.tutorial.beans.propertyeditors.StringArrayPropertyEditor;
import com.tutorial.beans.propertyeditors.TimeZoneEditor;
import com.tutorial.beans.propertyeditors.URIEditor;
import com.tutorial.beans.propertyeditors.URLEditor;
import com.tutorial.beans.propertyeditors.UUIDEditor;
...
public class PropertyEditorRegistrySupport implements PropertyEditorRegistry {
	...
	private Set<PropertyEditor> sharedEditor;
	...
	private void createDefaultEditors() {
		...
		// Default instance of collection editors.
		// Can be overridden by registering custom instance of those as custom editors.
		...
		// Spring's CustomBooleanEditor accepts more flag value than the JDK's default editor.
		this.defaultEditors.put(boolean.class, new CustomBooleanEditor(false));
		this.defaultEditors.put(Boolean.class, new CustomBooleanEditor(true));
		
		// The JDK  does not contain default editors for number wrapper type. 
	...
	public void registerCustomEditor(Class<?> requiredType, String propertyPath, PropertyEditor propertyEditor) {
		...
		if(propertyPath != null) {
			if(this.customEditorForPath == null) {
				this.customEditorForPath = new LinkedHashMap<String, CustomEditorHolder>(16);
			}
			this.customEditorForPath.put(propertyPath, new CustomEditorHolder(propertyEditor, requiredType));
		} else {
	...
	public boolean isSharedEditor(PropertyEditor propertyEditor) {
		return (this.sharedEditor != null && this.sharedEditor.contains(propertyEditor));
	}
	...
	public boolean hasCustomEditorForElement(Class<?> elementType, String propertyPath) {
		if(propertyPath != null && this.customEditorForPath != null) {
			for(Map.Entry<String, CustomEditorHolder> entry : this.customEditorForPath.entrySet()) {
	...
	private PropertyEditor getCustomEditor(Class<?> requiredType) {
		...
		if(editor == null) {
			...
			if(editor == null) {
				// Find editor for superclass or interfaces .
	...
	protected Class<?> guessPropertyTypeFromEditors(String propertyName) {
		if(this.customEditorForPath != null) {
			CustomEditorHolder editor = this.customEditorForPath.get(propertyName);
			if(editor == null) {
				List<String> strippedPaths = new LinkedList<String>();
				addStrippedPropertyPaths(strippedPaths, "", propertyName);
				for(Iterator<String> it = strippedPaths.iterator(); it.hasNext() && editor == null;) {
					String strippedName = it.next();
					editor = this.customEditorForPath.get(strippedName);
				}
			}
			if(editor != null) {
				return editor.getRegisterType();
			}
		}
		return null;
	}
				...
				if(nestedProperty != null) {
					...
					if(pos != -1) {
						...
						if(editorNestedProperty.equals(nestedProperty) || 
							...
							target.registerCustomEditor(editorHolder.getRegisterType(), 
									editorNestedPath, editorHolder.getPropertyEditor());
						}
					}
				} else {
					target.registerCustomEditor(editorHolder.getRegisterType(), editorPath, editorHolder.getPropertyEditor());
				}
			}
		}
	}
	...
	private PropertyEditor getCustomEditor(String propertyPath, Class<?> requiredType) {
		CustomEditorHolder holder = this.customEditorForPath.get(propertyPath);
	...
	private static class CustomEditorHolder {
		...
		private final Class<?> registerType;
		
		private CustomEditorHolder(PropertyEditor propertyEditor, Class<?> registerdType) {
			this.propertyEditor = propertyEditor;
			this.registerType = registerdType;
		}
		...
		private PropertyEditor getPropertyEditor(Class<?> requiredType) {
			...
			if (this.registerType == null ||
					(requiredType != null &&
					(ClassUtils.isAssignable(this.registerType, requiredType) ||
					ClassUtils.isAssignable(requiredType, this.registerType))) ||
					(requiredType == null &&
					(!Collection.class.isAssignableFrom(this.registerType) && !this.registerType.isArray()))) {
