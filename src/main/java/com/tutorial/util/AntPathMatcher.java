...
public class AntPathMatcher implements PathMatcher {
	...
	protected boolean doMatch(String pattern, String path, boolean fullMatch, 
			Map<String, String> uriTemplateVariable) {
		...
		while(pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
			...
			if(!matchStrings(patDir, pathDirs[pathIdxStart], uriTemplateVariable)) {
				return false;
			}
			pattIdxStart++;
			pathIdxStart++;
		...
		if(pathIdxStart > pathIdxEnd) {
			//Path is exhausted, only match if rest of the pattern is * or **'s 
		...
		while(pattIdxStart <= pattIdxEnd && pathIdxStart <= pathIdxEnd) {
			...
			if(!matchStrings(patDir, pathDirs[pathIdxEnd], uriTemplateVariable)) {
				return false;
			}
			pattIdxEnd--;
			pathIdxEnd--;
		...
		while(pattIdxStart != pattIdxEnd && pathIdxStart <= pathIdxEnd) {
			...
			for(int i = 0; i <= strLength - patLength; i++) {
				for(int j = 0; j < patLength; j++) {
					...
					String subStr = pathDirs[pathIdxStart + j + i];
					if(!matchStrings(subPat, subStr, uriTemplateVariable)) {
						continue strLoop;
					}
				}
	...
	private static class AntPatternComparator implements Comparator<String> {
		...
		public int compare(String o1, String o2) {
			...
			boolean pattern1EqualsPath = o1.equals(path);
			boolean pattern2EqualsPath = o2.equals(path);
			...
			int wildCardCount1 = getWildCardCount(o1);
			int wildCardCount2 = getWildCardCount(o2);
			
			int bracketCount1 = StringUtils.countOccurrencesOf(o1, "{");
			int bracketCount2 = StringUtils.countOccurrencesOf(o2, "{");
			...
			int pattern1Length = getPatternLength(o1);
			int pattern2Length = getPatternLength(o2);
