...
import com.tutorial.util.MethodInvoker;
...
public class ReflectionHelper {
	 ...
	 */
	public static ArgumentsMatchInfo compareArguments(List<TypeDescriptor> expectedArgTypes,
	 ...
	 * @return a MatchInfo object indicating what kind of match it was or null if it was not a match
	 */
	public static ArgumentsMatchInfo compareArgumentsVarargs(List<TypeDescriptor> expectedArgTypes,
			List<TypeDescriptor> suppliedArgTypes, TypeConverter typeConverter) {
		...
		// Check up until the varargs arguments.
		...
		for(int i = 0; i < argCountUpToVarargs; i++) {
			TypeDescriptor suppliedArg = suppliedArgTypes.get(i);
			TypeDescriptor expectedArg = expectedArgTypes.get(i);
			if(suppliedArg == null) {
				if(expectedArg.isPrimitive()) {
		...
		// If already confirmed it cann't be a match, then return;
		if(match == null) {
			return null;
		}
		...
		if(suppliedArgTypes.size() == expectedArgTypes.size() && 
			...
			// All remaining parameters must be of this type of convertable to this type.
	 ...
	 */
	public static void convertArguments(TypeConverter converter, Object[] arguments, Object methodOrCtor,
	...
	public static Object[] setupArgumentsForVarargsInvocation(Class<?>[] requiredParameterTypes, Object... args) {
		...
		if(parameterCount != argumentCount ||
			...
			// Now sort out the final arguments, which is the varargs one. Before entering this 
			// method the arguments should have been converted to the box from of the required type.
			...
			if(componentType.isPrimitive()) {
				if(componentType == Integer.TYPE) {
					...
					for(int i = 0; i < arraySize; i++) {
						repackagedArguments[i] = ((Float) args[parameterCount - 1 + i]).floatValue();
					}
					newArgs[newArgs.length - 1] = repackagedArguments;
		...
		for(int argPosition = 0; argPosition < arguments.length; argPosition++) {
			...
			try {
				...
				if(argument != null && !targetType.getObjectType().isInstance(argument)) {
					...
					arguments[argPosition] = converter.convertValue(argument, 
							TypeDescriptor.forObject(argument), targetType); 
				}
				...
				if(e instanceof SpelEvaluationException) {
					...
					throw new SpelEvaluationException(e, SpelMessage.TYPE_CONVERSION_ERROR,
							arguments[argPosition].getClass().getName(), targetType);
				}
			}
		}
	}
	 ...
	 */
	public static int getTypeDifferenceWeight(List<TypeDescriptor> paramTypes,
		...
		for(int i = 0, max = paramTypes.size(); i < max; i++) {
			...
			if(argType != null) {
				Class<?> paramTypeClazz = paramType.getClass();
	...
	public static enum ArgsMatchKind {
		...
		EXACT,
		// A close match is where the parameter types either exactly match or are assignment compatible with 
		// the method/constructor being invoked.
		CLOSE,
