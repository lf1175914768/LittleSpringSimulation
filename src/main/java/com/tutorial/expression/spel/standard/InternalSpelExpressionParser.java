...
import com.tutorial.expression.spel.ast.Assign;
import com.tutorial.expression.spel.ast.BeanReference;
import com.tutorial.expression.spel.ast.BooleanLiteral;
import com.tutorial.expression.spel.ast.CompoundExpression;
import com.tutorial.expression.spel.ast.ConstructorReference;
import com.tutorial.expression.spel.ast.Elvis;
import com.tutorial.expression.spel.ast.FunctionReference;
import com.tutorial.expression.spel.ast.Identifier;
import com.tutorial.expression.spel.ast.Indexer;
import com.tutorial.expression.spel.ast.InlineList;
import com.tutorial.expression.spel.ast.Literal;
import com.tutorial.expression.spel.ast.MethodReference;
import com.tutorial.expression.spel.ast.NullLiteral;
import com.tutorial.expression.spel.ast.OpAnd;
import com.tutorial.expression.spel.ast.OpDivide;
import com.tutorial.expression.spel.ast.OpEQ;
import com.tutorial.expression.spel.ast.OpGE;
import com.tutorial.expression.spel.ast.OpGT;
import com.tutorial.expression.spel.ast.OpLE;
import com.tutorial.expression.spel.ast.OpLT;
import com.tutorial.expression.spel.ast.OpMinus;
import com.tutorial.expression.spel.ast.OpModulus;
import com.tutorial.expression.spel.ast.OpMultiply;
import com.tutorial.expression.spel.ast.OpNE;
import com.tutorial.expression.spel.ast.OpOr;
import com.tutorial.expression.spel.ast.OpPlus;
import com.tutorial.expression.spel.ast.OperatorInstanceof;
import com.tutorial.expression.spel.ast.OperatorMatches;
import com.tutorial.expression.spel.ast.OperatorNot;
import com.tutorial.expression.spel.ast.OperatorPower;
import com.tutorial.expression.spel.ast.Projection;
import com.tutorial.expression.spel.ast.PropertyOrFieldReference;
import com.tutorial.expression.spel.ast.QualifiedIdentifier;
import com.tutorial.expression.spel.ast.Selection;
import com.tutorial.expression.spel.ast.SpelNodeImpl;
import com.tutorial.expression.spel.ast.StringLiteral;
import com.tutorial.expression.spel.ast.Ternary;
import com.tutorial.expression.spel.ast.TypeReference;
import com.tutorial.expression.spel.ast.VariableReference;
...
public class InternalSpelExpressionParser extends TemplateAwareExpressionParser {
	...
	private SpelNodeImpl eatRelationalExpression() {
		...
		if (relationalOperatorToken != null) {
			...
			if (tk == TokenKind.INSTANCEOF) {
				...
				return new com.tutorial.expression.spel.ast.OperatorBetween(toPos(t), expr, rhExpr);
			}
		}
		return expr;
	}
	...
	private SpelNodeImpl eatPrimaryExpression() {
		...
		if (nodes.size() == 1) {
			...
			return new CompoundExpression(toPos(start.getStartPosition(), nodes.get(nodes.size() - 1).getEndPosition()),
	...
	private boolean maybeEatFunctionOrVar() {
		...
		if (args == null) {
			...
			push(new FunctionReference(functionOrVariableName.data, toPos(t.startpos, functionOrVariableName.endpos),
					args));
			return true;
		}
	}
	...
	private void eatConstructorArgs(List<SpelNodeImpl> accumulatedArguments) {
		if (!peekToken(TokenKind.LPAREN)) {
			throw new InternalParseException(new SpelParseException(expressionString, positionOf(peekToken()),
	...
	private SpelNodeImpl eatPossiblyQualifiedId() {
		...
		return new QualifiedIdentifier(
				toPos(startnode.startpos, qualifiedIdPieces.get(qualifiedIdPieces.size() - 1).getEndPosition()),
	...
	private boolean maybeEatMethodOrProperty(boolean nullSafeNavigation) {
		if (peekToken(TokenKind.IDENTIFIER)) {
			...
			if (args == null) {
				...
				// methodreference
				push(new MethodReference(nullSafeNavigation, methodOrPropertyName.data, toPos(methodOrPropertyName),
						args));
				// TODO what is the end position for a method reference? the
				// name or the last arg?
				return true;
			}
		}
		return false;
	...
	private Token eatToken(TokenKind expectedKind) {
		...
		if (t.kind != expectedKind) {
			raiseInternalException(t.startpos, SpelMessage.NOT_EXPECTED_TOKEN, expectedKind.toString().toLowerCase(),
