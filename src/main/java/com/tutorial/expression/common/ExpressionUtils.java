...
public abstract class ExpressionUtils {
	 ...
	 */
	public static <T> T convert(EvaluationContext context, Object value,
			Class<T> targetType) throws EvaluationException {
	...
	public static <T> T convertTypedValue(EvaluationContext context, TypedValue typedValue, Class<T> targetType) {
		...
		if(context != null) {
			return (T) context.getTypeConverter().convertValue(value, 
					typedValue.getTypeDescriptor(), TypeDescriptor.valueOf(targetType));
		}
		throw new EvaluationException("Cann't convert value '" + value + "' to type '" + targetType.getName() + "'");
	}

	/**
	 * Attempt to convert a typed value to an int using the supplied type converter.
	 */
	public static int toInt(TypeConverter typeConverter, TypedValue o) {
		return (Integer) typeConverter.convertValue(o.getValue(),
				o.getTypeDescriptor(), TypeDescriptor.valueOf(Integer.class));
	}
