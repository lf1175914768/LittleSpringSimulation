...
public abstract class AbstractEnvironment implements ConfigurableEnvironment {
	...
	public AbstractEnvironment() {
		...
		logger.debug(String.format("Initializing %s with PropertySources %s", name, propertySources));
	}
	 ...
	 * an {@link com.tutorial.context.ApplicationContextInitializer
	 ...
	 * @see com.tutorial.context.ApplicationContextInitializer
	 */
	protected void customizePropertySources(MutablePropertySources propertySources) {
	}
	...
	protected Set<String> doGetDefaultProfiles() {
		if(this.defaultProfiles.equals(this.getReservedDefaultProfiles())) {
			...
			if(StringUtils.hasText(profiles)) {
				this.setDefaultProfiles(StringUtils.commaDelimitedListToStringArray(
						StringUtils.trimAllWhitespace(profiles)));
			}
		}
	...
	public Map<String, Object> getSystemEnvironment() {
		...
		try {
			...
			systemEnvironment = new ReadOnlySystemAttributeMap() {
				@Override
					...
					try {
						...
						if(logger.isInfoEnabled()) {
							logger.info(String.format("Caught AccessControlException when accessing system environment variable " + 
								"[%s]; its value will be returned [null]. Reason : [%s]", variableName, e.getMessage()));
						}
						return null;
					}
				}
	...
	public Map<String, Object> getSystemProperties() {
		...
		try {
			...
			systemProperties = new ReadOnlySystemAttributeMap() {
				@Override
				protected String getSystemAttribute(String attributeName) {
					try {
						...
						if (logger.isInfoEnabled()) {
							logger.info(String.format("Caught AccessControlException when accessing system environment variable " +
									"[%s]; its value will be returned [null]. Reason: %s", attributeName, e.getMessage()));
						}
						return null;
					}
				}
