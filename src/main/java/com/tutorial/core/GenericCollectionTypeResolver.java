...
public abstract class GenericCollectionTypeResolver {
	 ...
	 */
	private static Class<?> extractType(Type type, Class<?> source, int typeIndex, Map<TypeVariable<?>, Type> typeVariableMap,
		...
		if(resolvedType instanceof ParameterizedType) {
			...
			return extractType(compType, source, typeIndex, typeVariableMap, 
					typeIndexesPerLevel, nestingLevel, currentLevel + 1);
		}
		else {
			return null;
		}		
	 ...
	 * @return the generic type as Class, or <code>null</code> if none
	 */
	private static Class<?> extractTypeFromParameterizedType(ParameterizedType ptype, Class<?> source,
			int typeIndex, Map<TypeVariable<?>, Type> typeVariableMap, Map<Integer, Integer> typeIndexesPerLevel,
