...
import com.tutorial.core.io.FileSystemResource;
import com.tutorial.core.io.Resource;
import com.tutorial.core.io.ResourceLoader;
import com.tutorial.core.io.UrlResource;
import com.tutorial.core.io.VfsResource;
import com.tutorial.util.AntPathMatcher;
import com.tutorial.util.Assert;
import com.tutorial.util.PathMatcher;
 ...
 * target {@link com.tutorial.core.io.Resource}, or alternatively
 * may contain the special "<code>classpath*:</code>" prefix and/or
 * internal Ant-style regular expressions (matched using Spring's
 * {@link com.tutorial.util.AntPathMatcher} utility).
 ...
 * @see com.tutorial.util.AntPathMatcher
 * @see com.tutorial.core.io.ResourceLoader#getResource(String)
...
public class PathMatchingResourcePatternResolver implements ResourcePatternResolver {
	 ...
	 * @see com.tutorial.util.PathMatcher
	 */
		...
		try {
			...
			if(logger.isWarnEnabled()) {
				logger.warn("Cannot search for matching files underneath " + rootDirResource + 
						" because it does not correspond to a directory in the file system", e);
			}
			return Collections.emptySet();
		}
		return doFindMatchingFilesSystemResource(rootDir, subPattern);
	}
	 ...
	 * @see com.tutorial.util.PathMatcher
	 */
	protected Set<Resource> doFindMatchingFilesSystemResource(File rootDir, String subPattern) 
			throws IOException {
		if(logger.isDebugEnabled()) {
	...
	protected Set<File> retrieveMatchingFiles(File rootDir, String pattern) 
		...
		if(!rootDir.exists()) {
			...
			if(logger.isDebugEnabled()) {
				logger.debug("Skipping [" + rootDir.getAbsolutePath() + "] because it not exists");
			}
			return Collections.emptySet();
		}
		if(!rootDir.isDirectory()) {
			...
			if(logger.isWarnEnabled()) {
				logger.warn("Skip [" + rootDir.getAbsolutePath() + "] because it does not denote a directory ");
			}
			return Collections.emptySet();
		}
		if(!rootDir.canRead()) {
	 ...
	 * @see #doFindPathMatchingJarResources
	 * @see com.tutorial.util.ResourceUtils#isJarURL
	 */
	...
	private static class PatternVirtualFileVisitor implements InvocationHandler {
		...
		public void visit(Object resource) {
			if(this.pathMatcher.match(this.subPattern,
					VfsPatternUtils.getPath(resource).substring(this.rootPath.length()))) {
				this.resources.add(new VfsResource(resource));
			}
		}
