...
public class TypeDescriptor {
	...
	public TypeDescriptor getMapValueTypeDescriptor(Object value) {
		return narrow(value, getMapValueTypeDescriptor());
	}
	...
	public static Annotation[] nullSafeAnnotations(Annotation[] annotations) {
	...
	private TypeDescriptor(Class<?> type, TypeDescriptor elementTypeDescriptor) {
		this(type, elementTypeDescriptor, null, null, EMPTY_ANNOTATION_ARRAY);
	}
	...
	private void assertMap() {
		if(!isMap()) {
			throw new IllegalArgumentException("Not a java.util.Map");
		}
	}
	...
	public String toString() {
		...
		for(Annotation ann : anns) {
			sb.append("@").append(ann.annotationType().getName()).append(' ');
		}
		sb.append(ClassUtils.getQualifiedName(getType()));
		if(isMap()) {
			sb.append("<").append(wildCard(getMapKeyTypeDescriptor()));
			sb.append(", ").append(wildCard(getMapValueTypeDescriptor())).append(">");
		}
		else if(isCollection()) {
			sb.append("<").append(wildCard(getElementTypeDescriptor())).append(">");
		}
		return sb.toString();
	}

	private String wildCard(TypeDescriptor descriptor) {
