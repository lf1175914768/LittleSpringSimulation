...
public class ResourceBundleMessageSource extends AbstractMessageSource implements BeanClassLoaderAware {
	...
	protected String resolveCodeWithoutArguments(String code, Locale locale) {
		String result = null;
		for(int i = 0; i < this.basenames.length && result == null; i++) {
	 ...
	 */
	protected MessageFormat getMessageFormat(ResourceBundle bundle, String code,
		...
		synchronized(this.cachedResourceBundles) {
			...
			try {
				...
				if(logger.isWarnEnabled()) {
					logger.warn("ResourceBundle [" + basename + "] not found for messageSource: " + e.getMessage());
				}
