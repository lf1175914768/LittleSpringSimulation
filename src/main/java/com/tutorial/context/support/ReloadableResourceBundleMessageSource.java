			...
			if(this.fallbackToSystemLocale && !locale.equals(Locale.getDefault())) {
				...
				for(String fallbackFilename : fallbackFilenames) {
					if(!filenames.contains(fallbackFilename)) {
						// Entry for fallback locale that isn't already in filename list.
		...
		if(resource.exists()) {
			...
			if(this.cacheMillis >= 0) {
				...
				try {
					...
					// Probably a class with path resource: cache it forever.
					if(logger.isDebugEnabled()) {
						logger.debug(resource + " could not be resolved in the file system - assuming that is hasn't changed", e);
					}
					fileTimestamp = -1;
				}
			}
